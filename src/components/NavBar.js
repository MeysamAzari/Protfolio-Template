...
const CustomMobileLink = ({ href, title, className = "", toggle }) => {
  ...
  return (
    <button onClick={handleClick} href={href} className={`${className} relative group text-light dark:text-dark my-2`}>
      {title}
      <span className={`h-[1px] inline-block w-0 bg-dark absolute left-0 -bottom-0.5 
      group-hover:w-full transition-[width] ease duration-300 ${router.asPath === href ? 'w-full' : 'w-0'} bg-light
      dark:bg-dark`}>&nbsp;</span>
    </button>
  )
}
    ...
    md:px-12 sm:px-8'>
      ...
      z-30 bg-dark/90 dark:bg-light/75 rounded-lg backdrop-blur-md py-32'>
            <nav className='flex items-center flex-col justify-center'>
              ...
              <CustomMobileLink href="/projects" title="Projects" className='' toggle={handleClick}></CustomMobileLink>
              <CustomMobileLink href="/articles" title="Articles" className='' toggle={handleClick}></CustomMobileLink>
            </nav>
            <nav className='flex items-center justify-center flex-warp mt-2'>
              ...
              <motion.a href="/" target={"_blank"} whileHover={{ y: -2 }} whileTap={{ scale: 0.9 }} className='w-6 ml-3 sm:mx-1'><LinkedInIcon /></motion.a>
              <button onClick={() => setMode(mode === "light" ? "dark" : "light")}
                className={`ml-3 flex items-center justify-center rounded-full p-1 
          ${mode === "light" ? "bg-dark text-light" : " bg-light text-dark"}`}>
                {
